...
employee_df = spark.sparkContext.parallelize(employeeData).toDF(employeeColumn)
job_df = spark.sparkContext.parallelize(jobData).toDF(jobColumn)
salary_df = spark.sparkContext.parallelize(salaryData).toDF(salaryColumn)

# 2) Rename the columns by using using capital letters and replace '_' with space 

for column in employee_df.columns:
    employee_df = employee_df.withColumnRenamed(column,column.replace("_"," "))
print(employee_df.columns)

for column in job_df.columns:
    job_df = job_df.withColumnRenamed(column,column.replace("_"," "))
print(job_df.columns)
    
for column in salary_df.columns:
    salary_df = salary_df.withColumnRenamed(column,column.replace("_"," "))
print(salary_df.columns)

# 3) Format birth_date as 01.Jan.2021 
employee_df = employee_df.withColumn("birth date",F.date_format(F.col("birth date"),"d.MMM.y"))
employee_df.show()
...
concatenation = (F.concat(F.substring(F.col("first name"),0,2),F.col("last name"),F.lit("@company.com")))
employee_df = employee_df.withColumn("email",concatenation)
employee_df.show()
